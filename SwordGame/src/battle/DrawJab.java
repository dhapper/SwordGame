...
public class DrawJab {
	...
	public void drawFrame(PlayerBattleState attacker, PlayerBattleState defender, Graphics g) {
		...
		int movement = 2 * drawBattle.width/24;
		...
		if(drawBattle.frameNum <= retreatEnd)
			for(int i = retreatStart; i <= retreatEnd; i++) {
				if(drawBattle.frameNum == i) {
					drawBattle.drawCharacter.paintPlayer(attacker, 0, movement*(8-i), g);
					drawBattle.drawCharacter.paintArmour(attacker, 0, movement*(8-i), g);
					drawBattle.drawCharacter.paintShield(attacker, movement*(8-i), 0, g);
					drawBattle.drawCharacter.paintSword(attacker, movement*(8-i), 0, drawBattle.swordAngle, g);
				}
			}
	}
}
